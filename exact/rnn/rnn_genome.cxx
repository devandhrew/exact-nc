...
void RNN_Genome::write_predictions(const vector<string> &input_filenames, const vector<double> &parameters, const vector< vector< vector<double> > > &inputs, const vector< vector< vector<double> > > &outputs, const int32_t &threshhold) {
    RNN *rnn = get_rnn();
    rnn->set_weights(parameters);
    ...
    for (uint32_t i = 0; i < inputs.size(); i++) {
        ...
        rnn->write_predictions(output_filename, input_parameter_names, output_parameter_names, inputs[i], outputs[i], use_dropout, dropout_probability, threshhold);
    }
