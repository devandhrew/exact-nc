...
//Method for printing neuron_coverage
void print_node_coverage(int total_node, int covered_neurons, float neuron_coverage){
	cout << "Number of Node: " << total_node << " Number of covered nodes: " << covered_neurons << " Neuron Coverage: " << neuron_coverage << endl;

}

map<std::tuple<double,int32_t>,bool> init_model_dict(vector<RNN_Node_Interface*> &nodes){//Dict creation


    map<std::tuple<double,int32_t>,bool>model_dict;//Dict creation


    //Create the model with key [depth(layer),innovationnum] = false
...
float RNN::update_node_coverage(int32_t time, map<std::tuple<double,int32_t>,bool>model_dict, const double &threshhold){

    //Neuron Coverage code
    for(int32_t i = 0; i < nodes.size() ; i++){
        RNN_Node_Interface* node = nodes.at(i); 
	std::tuple<double, int32_t>key(nodes.at(i)->get_depth(),nodes.at(i)->get_innovation_number());
	if(model_dict.count(key) == 1){
	    if( abs(node->output_values.at(time)) > threshhold && !model_dict[key]){ //if the output is greater than the threshold and the value at the key is false
	        model_dict[key] = true;
	    }
        }
    }
	
    //Get the number of nodes that have true for activation
...
void RNN::forward_pass(const vector< vector<double> > &series_data, bool using_dropout, bool training, double dropout_probability, const double &threshhold) {
    
    map<std::tuple<double,int32_t>,bool>model_dict = init_model_dict(nodes);//Dict creation
	...
	neuron_coverage.push_back(update_node_coverage(time, model_dict, threshhold));
    }  
    float average_nc = std::reduce(neuron_coverage.begin(), neuron_coverage.end())/ neuron_coverage.size();
    cout << "Average Neuron Coverage: " << average_nc * 100 << endl;
}
...
void RNN::write_predictions(string output_filename, const vector<string> &input_parameter_names, const vector<string> &output_parameter_names, const vector< vector<double> > &series_data, const vector< vector<double> > &expected_outputs, bool using_dropout, double dropout_probability, const double &threshhold){
    forward_pass(series_data, using_dropout, false, dropout_probability,threshhold);
